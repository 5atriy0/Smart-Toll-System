  ...
  return (
    <div className="w-64 border-r bg-card/10 backdrop-blur-md hidden md:flex flex-col h-full items-start py-8 px-4 transition-all">
      <div className="flex items-center gap-2 px-2 mb-8 w-full">
        ...
        <span className="font-bold text-lg text-foreground tracking-wide">Smart Toll</span>
      </div>

      <nav className="w-full flex space-y-2 flex-col">
        <NavItem icon={<LayoutDashboard size={20}/>} href="/" label="Dashboard" active />
        <NavItem icon={<Users size={20}/>} href="#" label="User Management" />
        <NavItem icon={<Activity size={20}/>} href="#" label="Transaction Logs" />
        <NavItem icon={<BarChart3 size={20}/>} href="#" label="Big Data Analytics" />
        <NavItem icon={<Settings size={20}/>} href="#" label="System Settings" />
      </nav>

      <div className="mt-auto px-4 w-full">
        <div className="p-4 rounded-xl bg-primary/10 border border-primary/20 flex flex-col gap-2">
          <span className="text-sm font-semibold text-primary">System Online</span>
          <span className="text-xs text-muted-foreground">ESP32 Gateway Connected</span>
        </div>
      </div>
    </div>
  )
